...
func GetOperation(ctx context.Context, db *sql.DB, id int) (entities.Operation, error) {
...
func AddOperation(ctx context.Context, db *sql.DB, value *entities.Operation) (int, error) {
	var id int
...
func GetAllOperations(ctx context.Context, db *sql.DB) ([]entities.Operation, error) {
...
func UpdateLeftOperation(ctx context.Context, db *sql.DB, id int, leftData float64) error {
	_, err := db.ExecContext(ctx, "UPDATE operations SET left_data = $1, left_is_ready = 1 WHERE id = $2", leftData, id)
	return err
}

func UpdateRightOperation(ctx context.Context, db *sql.DB, id int, rightData float64) error {
	_, err := db.ExecContext(ctx, "UPDATE operations SET right_data = $1, right_is_ready = 1 WHERE id = $2", rightData, id)
	return err
}

func UpdateFatherOperation(ctx context.Context, db *sql.DB, id, fatherId, side int) error {
	_, err := db.ExecContext(ctx, "UPDATE operations SET father_id = $1, son_side = $2 WHERE id = $3", fatherId, side, id)
	return err
}

func DeleteOperation(ctx context.Context, db *sql.DB, id int) error {
	_, err := db.ExecContext(ctx, "DELETE FROM operations WHERE id = $1", id)
	return err
}

func IsReadyToExecuteOperation(ctx context.Context, db *sql.DB, id int) (bool, error) {
